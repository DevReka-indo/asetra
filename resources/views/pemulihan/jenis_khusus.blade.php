...
@section('title', 'Pemulihan Jenis Aset Khusus')

@section('content')
<div class="container-fluid px-1 py-0 mt-0">
    <div class="mb-4">
        <h3 class="fw-bold mb-0">Pemulihan Kode Aset Khusus</h3>
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('pemulihan.jenis-khusus') }}" class="row g-2 align-items-end">
                {{-- Entries --}}
                <div class="col-md-1">
                ...
                <table class="table table-hover align-middle">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th>Kode Aset Khusus</th>
                            <th>Nama Aset Khusus</th>
                            <th>Tgl Dihapus</th>
                    ...
                    <tbody>
                        @forelse ($dataKhusus as $i => $khusus)
                            <tr>
                                ...
                                <td class="fw-bold text-primary">{{ $khusus->kode_khusus }}</td>
                                <td>{{ $khusus->jenis_aset }}</td>
                                <td>{{ $khusus->deleted_at ? $khusus->deleted_at->format('d/m/Y H:i') : '-' }}</td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center align-items-center gap-2">
                                        {{-- RESTORE BUTTON --}}
                                        <button type="button" class="btn btn-success btn-sm rounded-circle text-white border-0" 
                                            style="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;"
                                            title="Pulihkan" data-bs-toggle="modal" data-bs-target="#restoreKhususModal{{ $khusus->id }}">
                                            <i class="fas fa-refresh"></i>
                                        </button>
                                        {{-- FORCE DELETE BUTTON --}}
                                        <button type="button" class="btn btn-danger btn-sm rounded-circle text-white border-0" 
                                            style="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;"
                                            title="Hapus Permanen" data-bs-toggle="modal" data-bs-target="#forceDeleteKhususModal{{ $khusus->id }}">
...
{{-- MODAL RESTORE --}}
<div class="modal fade" id="restoreKhususModal{{ $khusus->id }}" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-body p-5 text-center bg-light">
                ...
                <p class="text-muted mb-3" style="font-size: 1rem;">
                    Anda yakin ingin memulihkan data <strong class="text-success fs-5">{{ $khusus->kode_khusus }} - {{ $khusus->jenis_aset }}</strong>?
                </p>
                <div class="d-flex justify-content-center gap-3">
                    <form action="{{ route('pemulihan.jenis-khusus.restore', $khusus->id) }}" method="POST" class="w-100 d-flex justify-content-center gap-3">
                        @csrf 
                        @method('PUT')
...
{{-- MODAL FORCE DELETE --}}
<div class="modal fade" id="forceDeleteKhususModal{{ $khusus->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-body p-5 text-center bg-light">
                <div class="d-inline-flex align-items-center justify-content-center rounded-4 mb-4" style="width: 80px; height: 80px; background-color: #f1f3f5;">
                ...
                <p class="text-muted mb-3" style="font-size: 1rem;">
                    Hapus data <strong class="text-danger fs-5">{{ $khusus->kode_khusus }} - {{ $khusus->jenis_aset }}</strong> secara permanen?
                </p>
                
                <div class="alert alert-danger mb-4 text-start small border-0 shadow-sm rounded-3">
                    <i class="fas fa-info-circle me-1"></i><strong>Peringatan Keras:</strong> Tindakan ini tidak dapat dibatalkan. Semua data terkait berpotensi kehilangan referensi.
                </div>

                <div class="d-flex justify-content-center gap-3">
                    <form action="{{ route('pemulihan.jenis-khusus.force-delete', $khusus->id) }}" method="POST" class="w-100 d-flex justify-content-center gap-3">
                        @csrf 
                        @method('DELETE')
                        <button type="button" class="btn btn-light rounded-pill fw-bold py-2 shadow-sm border" style="width: 120px;" data-bs-dismiss="modal">Batalkan</button>
