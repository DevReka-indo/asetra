...
@section('title', 'Pemulihan Data Aset')
...
<div class="container-fluid px-1 py-0 mt-0">
    ...
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('pemulihan.data-aset') }}" class="row g-2 align-items-end">
                {{-- Entries --}}
                <div class="col-md-1">
                    ...
                    <tbody>
                        @forelse ($data as $i => $item)
                            <tr>
                                ...
                                <td class="fw-bold text-primary">{{ $item->nomor_aset }}</td>
                                <td>{{ $item->nama_aset }}</td>
                                <td>
                                    @if($item->dokumen_penghapusan)
                                        <a href="{{ asset('storage/' . $item->dokumen_penghapusan) }}" target="_blank" class="btn btn-sm btn-outline-info rounded-pill px-3 shadow-sm" title="Lihat Dokumen">
                                            <i class="fas fa-file-pdf me-1"></i> BAST
                                        </a>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="text-center py-4 text-muted">
                                    ...
                                    Tidak ada data aset di pemulihan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-body p-5 text-center bg-light">
                ...
                <p class="text-muted mb-3" style="font-size: 1rem;">
                    Anda yakin ingin memulihkan aset <strong class="text-success fs-5">{{ $item->nomor_aset }} - {{ $item->nama_aset }}</strong>?
                </p>
                <div class="d-flex justify-content-center gap-3">
                    <form action="{{ route('pemulihan.data-aset.restore', $item->id) }}" method="POST" class="w-100 d-flex justify-content-center gap-3">
                        @csrf 
                        @method('PUT')
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-body p-5 text-center bg-light">
                ...
                <p class="text-muted mb-3" style="font-size: 1rem;">
                    Hapus aset <strong class="text-danger fs-5">{{ $item->nomor_aset }} - {{ $item->nama_aset }}</strong> secara permanen?
                </p>
                
                <div class="alert alert-danger mb-4 text-start small border-0 shadow-sm rounded-3">
                    <i class="fas fa-info-circle me-1"></i><strong>Peringatan Keras:</strong> Tindakan ini tidak dapat dibatalkan. File PDF Dokumen Penghapusan dan Foto Aset juga akan dihapus.
                </div>

                <div class="d-flex justify-content-center gap-3">
                    <form action="{{ route('pemulihan.data-aset.force-delete', $item->id) }}" method="POST" class="w-100 d-flex justify-content-center gap-3">
                        @csrf 
                        @method('DELETE')
                        <button type="button" class="btn btn-light rounded-pill fw-bold py-2 shadow-sm border" style="width: 120px;" data-bs-dismiss="modal">Batalkan</button>
                        <button type="submit" class="btn btn-danger rounded-pill fw-bold py-2 shadow-sm" style="width: 140px;">Hapus Permanen</button>
                    </form>
                </div>
            </div>
        </div>
