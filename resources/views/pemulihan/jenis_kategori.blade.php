...
@section('title', 'Pemulihan Jenis Kategori')

@section('content')
<div class="container-fluid px-1 py-0 mt-0">
    <div class="mb-4">
        <h3 class="fw-bold mb-0">Pemulihan Jenis Kategori</h3>
        <p class="text-muted small">Data di bawah ini adalah Jenis Kategori yang telah dihapus. Anda dapat memulihkannya atau menghapusnya secara permanen.</p>
    </div>

    {{-- FILTER --}}
    <div class="card shadow-sm border-0 mb-4">
        <div class="card-body">
            <form method="GET" action="{{ route('pemulihan.jenis-kategori') }}" class="row g-2 align-items-end">
                {{-- Entries --}}
                <div class="col-md-1">
                ...
                <div class="col-md-3">
                    ...
                    <div class="input-group input-group-sm input-group-focus rounded-3">
                        ...
                        <input type="text" name="search" class="form-control border-0 shadow-none bg-transparent" placeholder="Cari nama jenis atau awalan..." value="{{ request('search') }}">
                    </div>
                </div>
            </form>
        </div>
                    ...
                    <tbody>
                        @forelse ($data as $i => $item)
                            <tr>
                                ...
                                <td class="fw-bold text-primary fs-5">{{ $item->kode_awalan }}</td>
                                ...
                                <td class="text-center">
                                    <div class="d-flex justify-content-center align-items-center gap-2">
                                        ...
                                        <button type="button" class="btn btn-success btn-sm rounded-circle text-white border-0" 
                                            ...
                                            title="Pulihkan" data-bs-toggle="modal" data-bs-target="#restoreModal{{ $item->id }}">
                                            <i class="fas fa-undo"></i>
                                        </button>
                                        {{-- FORCE DELETE BUTTON --}}
                                        <button type="button" class="btn btn-danger btn-sm rounded-circle text-white border-0" 
                                            style="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;"
                                            title="Hapus Permanen" data-bs-toggle="modal" data-bs-target="#forceDeleteModal{{ $item->id }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-4 text-muted">
                                    ...
                                    Tidak ada data Jenis Kategori di pemulihan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
{{-- MODAL RESTORE --}}
<div class="modal fade" id="restoreModal{{ $item->id }}" tabindex="-1" aria-hidden="true">
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-body p-5 text-center bg-light">
                ...
                <p class="text-muted mb-3" style="font-size: 1rem;">
                    Anda yakin ingin memulihkan Jenis Kategori <strong class="text-success fs-5">{{ $item->nama_jenis }}</strong>?
                </p>
                <div class="d-flex justify-content-center gap-3">
                    <form action="{{ route('pemulihan.jenis-kategori.restore', $item->id) }}" method="POST" class="w-100 d-flex justify-content-center gap-3">
                        @csrf 
                        @method('PUT')
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-body p-5 text-center bg-light">
                ...
                <p class="text-muted mb-3" style="font-size: 1rem;">
                    Hapus Jenis Kategori <strong class="text-danger fs-5">{{ $item->nama_jenis }}</strong> secara permanen?
                </p>
                
                <div class="alert alert-danger mb-4 text-start small border-0 shadow-sm rounded-3">
                    <i class="fas fa-info-circle me-1"></i><strong>Peringatan:</strong> Tindakan ini tidak dapat dibatalkan. Kategori aset yang masih merujuk ke jenis ini mungkin akan bermasalah jika jenisnya dihapus permanen.
                </div>

                <div class="d-flex justify-content-center gap-3">
                    <form action="{{ route('pemulihan.jenis-kategori.force-delete', $item->id) }}" method="POST" class="w-100 d-flex justify-content-center gap-3">
                        @csrf 
                        @method('DELETE')
                        <button type="button" class="btn btn-light rounded-pill fw-bold py-2 shadow-sm border" style="width: 120px;" data-bs-dismiss="modal">Batalkan</button>
